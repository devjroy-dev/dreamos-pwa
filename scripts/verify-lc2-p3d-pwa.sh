#!/usr/bin/env bash
# scripts/verify-lc2-p3d-pwa.sh — TDW CE-43 · LC-2 packet 3d (dreamos-pwa) · the founder's ONE verify command.
#
#   bash scripts/verify-lc2-p3d-pwa.sh
#
...
#   2  b82, b80, b81 and tdw37_hygiene_false_success (tdw37_leadgate_b_slot, amended by label at 3d, is in the named floor base and is judged by step 4)
#   3  rm -rf .next, then npx --no-install next build     the founder's gate (R-40.66)
#   4  bash scripts/run-floor.sh --delivery scripts/floor-manifest-lc2-p3d-pwa.txt --check
...
bash scripts/run-floor.sh --delivery scripts/floor-manifest-lc2-p3d-pwa.txt --check
rc=$?
