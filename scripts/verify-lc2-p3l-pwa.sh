#!/usr/bin/env bash
# scripts/verify-lc2-p3l-pwa.sh — TDW CE-43 · LC-2 packet 3l (dreamos-pwa) · the founder's ONE verify command.
#
#   bash scripts/verify-lc2-p3l-pwa.sh
#
...
#   4  bash scripts/run-floor.sh --delivery scripts/floor-manifest-lc2-p3l-pwa.txt --check
...
bash scripts/run-floor.sh --delivery scripts/floor-manifest-lc2-p3l-pwa.txt --check
rc=$?
