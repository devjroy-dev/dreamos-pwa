#!/usr/bin/env bash
# scripts/verify-lc2-p3l-docs-pwa.sh — TDW CE-43 · LC-2 packet 3m-docs (dreamos-pwa) · the founder's ONE verify command.
#
#   bash scripts/verify-lc2-p3l-docs-pwa.sh
#
...
#   4  bash scripts/run-floor.sh --delivery scripts/floor-manifest-lc2-p3l-docs-pwa.txt --check
...
bash scripts/run-floor.sh --delivery scripts/floor-manifest-lc2-p3l-docs-pwa.txt --check
rc=$?
