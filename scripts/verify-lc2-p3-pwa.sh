#!/usr/bin/env bash
# scripts/verify-lc2-p3-pwa.sh — TDW CE-43 · LC-2 packet 3 (dreamos-pwa) · the founder's ONE verify command.
#
#   bash scripts/verify-lc2-p3-pwa.sh
#
...
#   2  b82 (this packet), b80 and tdw37_hygiene_false_success (amended by label), b81
#   3  rm -rf .next, then npx --no-install next build     the founder's gate (R-40.66)
#   4  bash scripts/run-floor.sh --delivery scripts/floor-manifest-lc2-p3-pwa.txt --check
...
bash scripts/run-floor.sh --delivery scripts/floor-manifest-lc2-p3-pwa.txt --check
rc=$?
