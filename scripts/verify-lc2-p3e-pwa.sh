#!/usr/bin/env bash
# scripts/verify-lc2-p3e-pwa.sh — TDW CE-43 · LC-2 packet 3e (dreamos-pwa) · the founder's ONE verify command.
#
#   bash scripts/verify-lc2-p3e-pwa.sh
#
...
#   4  bash scripts/run-floor.sh --delivery scripts/floor-manifest-lc2-p3e-pwa.txt --check
...
bash scripts/run-floor.sh --delivery scripts/floor-manifest-lc2-p3e-pwa.txt --check
rc=$?
