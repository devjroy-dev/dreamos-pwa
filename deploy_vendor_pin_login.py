#!/usr/bin/env python3
"""
deploy_vendor_pin_login.py
Run this in /workspaces/dreamos-pwa

Creates app/vendor/pin-login/page.tsx — the vendor PIN entry page.
Same aesthetic as couple/pin-login: editorial images, carousel, frosted glass.
Uses vendor auth endpoints. Redirects to /vendor on success.

Also fixes landing page to route Makers to /vendor/pin-login after OTP
(instead of the black /vendor/login page).
"""
...
def write(p, t): path = BASE / p; path.parent.mkdir(parents=True, exist_ok=True); path.write_text(t)
...
print('\n── 1. app/vendor/pin-login/page.tsx (NEW) ──────────────────────────────')

write('app/vendor/pin-login/page.tsx', """\
'use client';
...
""")
print('  OK   [created app/vendor/pin-login/page.tsx]')

print('\n── 2. landing page — route Makers to /vendor/pin-login after OTP ───────')

# After OTP verify, vendors went to /vendor/login — now go to /vendor/pin-login
patch('app/(landing)/page.tsx',
    ...
    'post-OTP route for vendors'
)

# Sign-in with existing PIN → vendor/pin-login
patch('app/(landing)/page.tsx',
    "router.push(isVendor ? '/vendor/login' : '/couple/pin-login');",
    "router.push(isVendor ? '/vendor/pin-login' : '/couple/pin-login');",
    'sign-in route for vendors'
)

print('\n── TypeScript check ────────────────────────────────────────────────────')
result = subprocess.run(['npx', 'tsc', '--noEmit'], capture_output=True, text=True)
PRE = ['Cannot find module', 'jsx-runtime', 'Cannot find namespace', 'Cannot find name', 'TS7026', 'TS7006']
new_errors = [
    l for l in result.stdout.splitlines()
    if ('vendor/pin-login' in l or '(landing)/page' in l)
    and not any(p in l for p in PRE)
    and 'error TS' in l
]
if new_errors:
    print('  ERRORS:')
    for e in new_errors: print(f'    {e}')
    sys.exit(1)
else:
    print('  No new errors \u2713')

print('\n\u2705  Done. Commit with:')
print('  git add app/vendor/pin-login/page.tsx "app/(landing)/page.tsx"')
print('  git commit -m "feat(vendor): add pin-login page with editorial images, route landing → /vendor/pin-login"')
print('  git push')
