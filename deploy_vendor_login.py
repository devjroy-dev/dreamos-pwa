#!/usr/bin/env python3
"""
deploy_dreamai_login.py
Run this in /workspaces/dreamai

Rewrites app/vendor/login/page.tsx to add PIN step.
Flow: PHONE → OTP → PIN (enter existing) or SET_PIN (create new)

This makes thedreamai.in/vendor/login fully self-contained.
No cross-domain handoff from dreamos-pwa needed.
"""

import subprocess, sys
from pathlib import Path

PAGE = Path('app/vendor/login/page.tsx')
...
print('✓ wrote app/vendor/login/page.tsx')
...
errors = [l for l in result.stdout.splitlines() if 'vendor/login' in l and not any(p in l for p in PRE)]
if errors:
    print('  ERRORS:')
...
print('\n✅  Done. Commit with:')
print('  git add app/vendor/login/page.tsx')
print('  git commit -m "feat(login): add PIN step — self-contained auth, no cross-domain handoff"')
print('  git push')
