...
- **A real browser in the seat's container.** The tree pins `@sparticuz/chromium` **149.0.0** and `puppeteer-core` **25.9.0** as devDependencies (`package.json`). `chromium.executablePath()` gives the binary; launch headless with `chromium.args`. No CDN outside the registry is needed.
  - **CORRECTED AT CE-44 (F-44.4).** This line read `@sparticuz/chromium@131 puppeteer-core@23`, which are not the versions the tree carries. The pins above are read from `package.json` at `1db8a88e`.
  - **A SUBSTITUTION, RECORDED.** Seat LC-2t drove C-43.18 through the container's own **Playwright Chromium 1194** binary via `puppeteer-core`, rather than through `chromium.executablePath()`. Same instrument, different door; accepted by the chair at CE-44. A seat whose container already holds a browser need not install one.
