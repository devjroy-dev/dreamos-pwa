# TDW_20 · Concierge sitting 2 · D1 · the frames, for the chair's veto
...
**Veto column:** **all of it is the chair's** — R-41.98 amends R-41.32, and the founder has delegated every column including `V1`/`V2` (the vendor's decline card).

---

## 0 · The rule this was drawn under, and what it changed

**R-41.98, the founder's rule, now law:** no prose, no poetry. Plain, short, direct. A label is a noun phrase. A control is a verb. A refusal is one sentence carrying the reason and the next step. No clause chains, no em-dash tails, no three-line rows at 374.

The frames were drawn before the rule arrived and re-checked against it after. **Nine strings were rewritten.** They are marked ✎ below with what they were, so the chair can see the rule doing work rather than take my word that it did.

---

## 1 · Admin — the forward row at 374 (`F1`, `F2`, `F3`, `F4`, `F5`)
...
| **S2-16** | F3 | `They get one message to join. Her number stays with us until they do.` | ✎ was three clauses ending in a dash tail about the row saying so. Roadmap §7's refusal, said plainly. |
| **S2-17** | F4 | `Dev Roy Photography already has her enquiry` | **R-41.95.** Names the vendor rather than saying "this vendor". |
| **S2-18** | F4 | `Forwarding again adds this request to that lead and alerts her. No second lead is created.` | the override's consequence, stated before the tap. The second sentence is the estate's refusal, visible. |
| **S2-19** | F4 | `Forward anyway` / `Cancel` | controls |

**Struck from `F1` deliberately:** the `source tdw_assist` tail (S2-4), and any per-row rendering of `error_code`. **A bare Meta code never reaches the glass** — it goes to the log and the row, and the glass gets S2-5.

---

## 2 · Public intake (`P1-public-374`)

**The derivation the chair should check first.** R-41.94 rules there is no unauthenticated write: the public link *is* couple signup with the sheet pre-filled. She enters her phone, the bride-line OTP creates the couple, then the request writes with `couple_id` set and `origin='public'`. So this frame inherits **signup's** surface — the landing's own chrome at `app/(landing)/page.tsx:688–775` (`#0C0A09`, gold `#C9A84C`, Jost/DM Sans/Cormorant) — and **not** Wine Night, and **not** a light public page. If the chair reads R-41.94 differently, this frame is drawn on the wrong ground and I redraw it.

| # | string | why |
|---|---|---|
| **S2-20** | `Find my vendors` | display line. Matches seat A's ruled `#4`, so the caller and the app-user read one verb. |
| **S2-21** | `Tell us what you need. We find and book it.` | ✎ was seat A's longer `#3`. Cut to two sentences for a cold caller who has never seen TDW. |
| **S2-22** | `Wedding date` · `City` · `What you need` · `WhatsApp number` | labels, noun phrases |
| **S2-23** | `10 digits` | placeholder beside a fixed `+91`. `DEFAULT_COUNTRY` is India-only by R-41.34; the prefix is chrome, not a field. |
...
## 3 · Bride — amend after Send (`A1-s2-edit-374`, `A2-amend-374`)

| # | frame | string | why |
|---|---|---|---|
| **S2-26** | A1 | `Edit request` | control, verb. The only byte added to S2 as shipped. |
| **S2-27** | A1 | `Add a category or change a budget.` | one sentence, says what the control does |
| **S2-28** | A1 | `We're still looking.` | per-item waiting state, from `assistance_request_items.status` |
| **S2-29** | A1 | `Found` / `Looking` | the two item states she sees. `closed` is not shown to her. |
| **S2-30** | A2 | `Edit request` | sheet title |
...
## 4 · Vendor — the decline card (`V1-lead-374`, `V2-decline-374`)

Graphite, vendor lane. **All strings the chair's under R-41.98.** The card's existing rows are drawn as they ship (`body.tsx:179`'s `detail[]`) so the new control is seen in its real neighbourhood.

| # | frame | string | why |
|---|---|---|---|
| **S2-33** | V1 | `Not for me` | ✎ was `Decline this lead`. Three words, a vendor's own phrase, and it does not make her feel she is rejecting a person. |
| **S2-34** | V1 | `The Dream Wedding` | the `Source` row's value. ✎ was `tdw_assist`. **A register key on vendor glass is the F-41.62 class**, and seat C already filed it once this sprint. |
| **S2-35** | V2 | `Return this lead to The Dream Wedding?` | sheet title. "Return" not "decline": it goes somewhere, it is not destroyed. |
...
## 5 · The inbound-first door (`I1-inbound`) — design only

Not a surface. No string on it is product copy and none is numbered. It frames R-41.93 option B for the chair: the direction inverted, what it buys, what it costs, and the one open question — **whether the link is per-request or one standing TDW link.** The sheet deliberately does not choose.

The one ruled byte it records: the gate for a windowed free-form send is a **`flag.*`** key, never a `template.*` one, because there is no template to key on. Proposed name `flag.assist_inbound_door`, seeded off.
