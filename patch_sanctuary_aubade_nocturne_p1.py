#!/usr/bin/env python3
"""
patch_sanctuary_aubade_nocturne_p1.py
Frost · Direction 07 · Aubade & Nocturne · Phase 1 — Sanctuary static.

Run from the root of dreamos-pwa:
    python3 patch_sanctuary_aubade_nocturne_p1.py

What this does (Phase 1 only — static, no motion yet):
  1. app/layout.tsx        — add Fraunces, Italianno, JetBrains Mono to font link.
  2. lib/frost/tokens.ts   — add AUBADE Aubade-Nocturne token set + FF.aubade/.italianno/.mono.
  3. app/(frost)/frost/canvas/sanctuary/page.tsx — full replace with v2 surface:
        - Mineral teal numeral (280px) behind frost layers
        - Upper + lower frosted glass layers
        - Meridian arc SVG at top with static today-dot
        - Top bar: Aubade left-link / 26.v.26 right-link
        - Hero: mono greeting → Italianno "Hello, Priya" → saffron rule →
                Fraunces italic progress line ("A hundred and seventy-seven
                mornings between I will and I do") → mono date footer
        - Seven rows: I Dream · II Circle · III Muse · IV People · V Pages
                      · VI Moments · VII Journey
        - Pages right-side line: "a page is waiting" (placeholder until data wired)
        - Static candle dot (saffron) on Circle row — no flicker yet
        - Words-vs-numerals rule on count (under 100 = words, else digits)

Phase 1 ships static. Motion (heartbeat, drift, flicker, daily ribbon)
comes in Phase 2 after we screenshot and confirm the room is right.
...
def assert_in_repo():
    if not (ROOT / 'app' / '(frost)' / 'frost' / 'canvas' / 'sanctuary' / 'page.tsx').exists():
        sys.exit(f"ERROR: not in dreamos-pwa root. Run from the repo root that contains app/(frost)/frost/canvas/sanctuary/page.tsx. cwd={ROOT}")

def patch_file(path: str, old: str, new: str, label: str):
    p = ROOT / path
    s = p.read_text()
    if old not in s:
        sys.exit(f"ERROR [{label}]: anchor not found in {path}\n--- anchor ---\n{old[:200]}\n--------------")
    if s.count(old) > 1:
        sys.exit(f"ERROR [{label}]: anchor matches {s.count(old)} times in {path}. Need unique match.")
...
LAYOUT_NEW_LINK = '''<link
          href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,400;1,300;1,400&family=DM+Sans:wght@300;400;500&family=Jost:wght@200;300;400;500&family=Fraunces:ital,opsz,wght@0,9..144,200;0,9..144,300;0,9..144,400;1,9..144,300;1,9..144,400&family=Italianno&family=JetBrains+Mono:wght@200;300;400&display=swap"
          rel="stylesheet"
        />'''

# ─────────────────────────────────────────────────────────────────────────────
# 2. lib/frost/tokens.ts — add AUBADE token set + new font families
# ─────────────────────────────────────────────────────────────────────────────

TOKENS_OLD_FF = '''export const FF = {
  display: "'Cormorant Garamond', serif",
  body:    "'DM Sans', sans-serif",
  label:   "'Jost', sans-serif",
} as const;'''

TOKENS_NEW_FF = '''export const FF = {
  display:   "'Cormorant Garamond', serif",
  body:      "'DM Sans', sans-serif",
  label:     "'Jost', sans-serif",
...
# ─────────────────────────────────────────────────────────────────────────────
# 3. app/(frost)/frost/canvas/sanctuary/page.tsx — full replacement
# ─────────────────────────────────────────────────────────────────────────────

SANCTUARY_NEW = r"""'use client';
...
def main():
    ...
    print('\n  Phase 1 — Aubade & Nocturne · Sanctuary (static)\n')

    patch_file(
        'app/layout.tsx',
        LAYOUT_OLD_LINK,
        LAYOUT_NEW_LINK,
        'add Fraunces + Italianno + JetBrains Mono to font link'
    )
    ...
    patch_file(
        ...
        TOKENS_OLD_FF,
        TOKENS_NEW_FF,
        'extend FF (aubade/italianno/mono) + add AUBADE + AUBADE_GLASS'
    )

    replace_file(
        'app/(frost)/frost/canvas/sanctuary/page.tsx',
        SANCTUARY_NEW,
        'Sanctuary v2 — Aubade & Nocturne, static'
    )
    ...
    print('    npm run build')
    print('  Then commit + push, screenshot Sanctuary, send three states.\n')
