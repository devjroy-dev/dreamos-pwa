...
// Exact copy of real app/vendor/more/page.tsx
...
function MoreRow({ item }: { item: Item }) {
  const inner = (
    <>
      <span style={{ flexShrink:0, width:36, height:36, display:'flex', alignItems:'center', justifyContent:'center', fontFamily:F.display, fontWeight:400, fontSize:24, color: item.danger ? A.red : A.brassWarm, lineHeight:1 }}>{item.glyph}</span>
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ fontFamily:F.script, fontWeight:500, fontSize:19, color: item.danger ? A.red : A.ink, letterSpacing:'0.005em', lineHeight:1.15 }}>{item.label}</div>
        {item.description && <div style={{ fontFamily:F.script, fontStyle:'italic', fontWeight:300, fontSize:12, color:A.inkMute, marginTop:2, letterSpacing:'0.01em' }}>{item.description}</div>}
      </div>
      {!item.action && <Chevron />}
    </>
  );
  ...
  if (item.action) return <button type="button" onClick={item.action} style={{ ...rowStyle, borderBottom:'0.5px solid var(--atelier-card-border)' }}>{inner}</button>;
...
export default function DemoMorePage() {
  ...
  const ACCOUNT_ITEMS: Item[] = [
    ...
    { label:'Back to Studio', description:'', glyph:'→', action: () => router.push(`${base}/studio`) },
  ];
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflowY:'auto' }}>
      ...
      <div style={{ flex:1, paddingBottom:40 }}>
        ...
        {DISCOVER_ITEMS.map(item => <div key={item.label}><MoreRow item={item} /></div>)}
        <SectionLabel label="Team" />
        {TEAM_ITEMS.map(item => <div key={item.label}><MoreRow item={item} /></div>)}
        <SectionLabel label="Finance" />
        {FINANCE_ITEMS.map(item => <div key={item.label}><MoreRow item={item} /></div>)}
        <SectionLabel label="Account" />
        {ACCOUNT_ITEMS.map(item => <div key={item.label}><MoreRow item={item} /></div>)}
      </div>
    </div>
  );
}
