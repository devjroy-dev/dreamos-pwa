...
// Split out of `app/vendor/collab/page.tsx` so `/w/collab` and the surviving
...
export function CollabScreen({ vendorId, tier }: { vendorId: string; tier: string }) {
  ...
  const openResponses = (id: string) =>
    router.push((inShell ? '/w/collab/' : '/vendor/collab/') + id + '/responses');
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
      {/* ── THE PAGE HEADER ROW · TDS's §4-4 PRECEDENT, TREE-AWARE ───────────────
          ...
          surface at all. */}
      <div style={{ padding: inShell ? '10px 22px 10px' : '20px 22px 10px' }}>
