...
// Two routes render this module and neither owns it: `app/w/calendar/page.tsx` mounts it
// inside `WorklistShell`, and `app/vendor/calendar/page.tsx` survives as the untouched
// fallback and supplies the old `<Header/>` itself. The module is IMPORTED by both and
// copied by neither — two calendars would be two homes for every block, every hot date and
// every vetoed byte, drifting apart without either one erroring.
//
// ── WHY THE FILE SPLIT AT ALL, WHICH IS THE S2 LESSON PAID FOR ONCE ─────────
// `SliceShell` kept `import { Header }` and wrote `{chrome && <Header/>}`. It rendered
// correctly and STILL shipped the old masthead — its drawer, its /vendor rows, its banned
// bytes — into every crossed room's chunk. **A conditional does not remove a module from a
// bundle; only not importing it does.** So the body and the chrome cannot live in one file,
// and this is the split that guarantees it: the `Header` import is gone from this file and
// its absence is asserted, not intended.
//
// The shape mirrors the list family exactly — `app/vendor/list/[slice]/leads.tsx` is the
// body and `[slice]/page.tsx` is the route that carries the chrome. One precedent, one
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden', position: 'relative' }}>
      ...
      {/* ── THE MASTHEAD LEFT THIS FILE AT §4-2, AND `vendorName` LEFT WITH IT ──────
          ...
          (app/vendor/calendar/page.tsx) and by nothing else; under /w the shell's own
          chrome is already above this body.

          AND THE PROP WENT TOO, WHICH IS THE HALF THAT IS EASY TO SKIP. `vendorName` was
          read by exactly one thing — the Header — so once the mount left, the parameter was
          dead. An unused prop is not tidiness debt: it is a named, typed hole that the next
          reader fills, and then the body knows the vendor's name for no reason on a surface
          that must not print it. Derived, not assumed: the only occurrence in this file
      ...
      {/* Brass-key FAB */}
      {/* ── CE-39 S2/8 · F-39.4 · THE FOURTH SEAT, AND THE LAST ──────────────────
          THE FOUNDER FOUND THIS ONE TOO. F-39.4's hotfix gave Rooms, the list family and
          Notes one seat and left this file alone, so Calendar's button painted 46px at
          bottom 120 against the ruled 56 at 136 — ten smaller and sixteen lower than every
          other room, which is the exact complaint the hotfix was written to end.

          ⚠ AND THE CELL WAVED IT THROUGH ON PURPOSE. `b40` C39 skipped any bottom value
          containing an inShell ternary, because at F-38.59 that ternary WAS the cure — the
          comment this block replaced said so in its last line, naming C39 as the guard.
          F-39.4 moved the cure one rung up: the shell arm renders the one component and
          names no number at all. **So F-38.59's cure shape became F-39.4's defect shape**,
          and the cell went on exempting the thing it existed to catch. Ruled at CE-39 S2/8:
          that skip is RETIRED, a tree-aware ternary in a bottom value REDS from this date,
          and the only lawful exemption is the marker on the /vendor arm below.

          The seat itself lives in GRID.fab (lib/worklist/theme.ts) and reaches the shell
          arm through components/worklist/Fab.tsx. Nothing here names a number any more.
          The /vendor arm keeps its 82 — it clears the OLD shell's BottomNav and dies with
          that tree at Phase 7 — and DECLARES itself, so the exemption is claimed in the
          markup rather than inferred from how near the word inShell happens to sit. */}
      {inShell
        ? <Fab label="Add event" onClick={onAdd} />
        : (
          <button type="button" onClick={onAdd} aria-label="Add event" className="atelier-fab"
            data-tree="vendor" style={{
