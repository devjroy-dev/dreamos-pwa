...
'use client';
// app/vendor/portfolio/screen.tsx — THE PORTFOLIO MANAGER'S BODY, NO CHROME.
//
// ── §4-3 · PORTFOLIO CROSSES · R-38.11 · R-38.12 ───────────────────────────
// Two routes render this module and neither owns it: `app/w/portfolio/page.tsx` mounts it
// inside `WorklistShell`, and `app/vendor/portfolio/page.tsx` survives as the untouched
// fallback and supplies the old `<Header/>` itself. IMPORTED by both, copied by neither —
// and the §C singularity principle this manager was built under (ONE photo editor in the
// estate) is exactly the thing a copied body would break.
//
...
//
// ── THE MASTHEAD ROW SURVIVES BECAUSE ITS RIGHT HALF IS THE UPLOAD ACTION ──
// The row is `‹` + 「PORTFOLIO」 + 「+ Upload」. The chevron and the word are the old layout's
// chrome and retire inside the shell — WorklistShell prints the room's word and the two nav
// seats are the way back, so a second title and a second way out are the two-mastheads
// defect one level down. THE ROW ITSELF STAYS IN BOTH TREES, because Upload is this
// screen's one filled gold and its only way to add a photo. Retiring the row to retire the
// word would have taken the action with it; a spacer replaces the label's `flex: 1` so the
// action does not move under the thumb.
//
// ── `vendorName` LEFT WITH THE MOUNT ───────────────────────────────────────
// Read by exactly one thing, `<Header vendorName={…}/>`. Derived, not assumed: after the
// lift the only occurrence in this file was the signature. An unused prop is a named, typed
// hole the next reader fills.
//
// ── THE DECLARED GAPS, TWO OF THEM, NEITHER SWEPT ──────────────────────────
// ① The body did not cross typographically (R-38.12): the rooms' older type register and
//   F-38.22's colour literals. Captured, excluded from the render arm's tuple cell by name.
// ② THE SHEETS ARE `position:fixed`. The Instagram picker and the photo detail sheet
//   position against the VIEWPORT, not against this scroll column, so inside the shell they
//   sit over the dock and the nav. That is not new and it is not this crossing's doing —
//   calendar's sheets have done exactly this since §4-2 and the founder has walked them. It
//   is written down here rather than discovered a third time.
//
// ── THE ONE OUTBOUND LINK ──────────────────────────────────────────────────
// `/vendor/discover/preview` (「See your profile as couples do」) points at a carried surface
// with no registry entry. Already declared in `INTERIM_VENDOR_LINKS` since S1 — this
// crossing adds a second reachable site for an entry that was already counted.
...
export function PortfolioScreen({ vendorId }: { vendorId: string }) {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
      ...
      {/* ── THE ROW STAYS; ITS LEFT HALF IS THE OLD LAYOUT'S CHROME ──────────
          ...
          already owns the way out, and there is no chevron in the shell by construction —
          the two nav seats are the way back. Same contract Billing and Settings crossed
          under at S1, the list family at §4-1 and Couture beside this one.
