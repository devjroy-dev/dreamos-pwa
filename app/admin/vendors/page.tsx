    ...
    <>
      ...
      {confirmRevoke && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--role-scrim)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9998 }}>
          <div style={{ background: 'var(--atelier-card-bg)', border: '1px solid transparent', borderRadius: 6, padding: 28, maxWidth: 360, width: '90%' }}>
            ...
            <div style={{ fontFamily: '"DM Sans", sans-serif', fontWeight: 300, fontSize: 13, color: 'var(--atelier-ink-soft)', marginBottom: 24 }}>This will lock the Maker out of their dashboard immediately.</div>
      ...
      {showCreate && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--role-scrim)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 9997 }}>
          <div style={{ background: 'var(--atelier-card-bg)', borderRadius: '8px 8px 0 0', padding: 28, width: '100%', maxWidth: 480 }}>
            <div style={{ fontFamily: '"Cormorant Garamond", serif', fontWeight: 300, fontSize: 24, color: 'var(--atelier-ink)', marginBottom: 20 }}>Create Maker</div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 28 }}>
        <div>
          ...
          <div style={{ fontFamily: '"Cormorant Garamond", serif', fontWeight: 300, fontSize: 28, color: 'var(--atelier-ink)' }}>Makers</div>
        </div>
        <button onClick={() => setShowCreate(true)} style={{ background: 'var(--atelier-sheet-bg)', color: 'var(--atelier-page-bg)', border: 'none', padding: '11px 20px', fontFamily: '"Jost", sans-serif', fontWeight: 300, fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase', cursor: 'pointer', borderRadius: 2 }}>+ Create Maker</button>
      </div>

      {/* Search */}
      <input placeholder="Search makers…" value={search} onChange={e => setSearch(e.target.value)} style={{ width: '100%', maxWidth: 340, background: 'transparent', border: 'none', borderBottom: '1px solid transparent', outline: 'none', fontFamily: '"DM Sans", sans-serif', fontWeight: 300, fontSize: 13, color: 'var(--atelier-ink)', padding: '8px 0', marginBottom: 16 }} />
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[1,2,3,4].map(i => <div key={i} style={{ height: 52, background: 'var(--atelier-card-bg)', borderRadius: 4, border: '1px solid transparent', backgroundImage: 'linear-gradient(90deg, var(--atelier-page-bg) 25%, var(--atelier-section-bg) 50%, var(--atelier-page-bg) 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.5s infinite' }} />)}
        </div>
      ) : (
        <div style={{ background: 'var(--atelier-card-bg)', border: '1px solid transparent', borderRadius: 6, overflow: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 800 }}>
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={8} style={{ padding: '40px 14px', textAlign: 'center', fontFamily: '"DM Sans", sans-serif', fontWeight: 300, fontSize: 13, color: 'var(--atelier-ink-soft)' }}>Every dream starts with a Maker who dares to show up.</td></tr>
              )}
              {filtered.map(v => (
                <tr key={v.id} style={{ borderTop: '1px solid var(--atelier-card-border)' }}>
                  <td style={{ padding: '11px 14px', fontFamily: '"DM Sans", sans-serif', fontWeight: 400, fontSize: 13, color: 'var(--atelier-ink)' }}>{v.name || '—'}</td>
                  <td style={{ padding: '11px 14px', fontFamily: '"DM Sans", sans-serif', fontWeight: 300, fontSize: 12, color: 'var(--atelier-ink-soft)' }}>{v.category || '—'}</td>
                  <td style={{ padding: '11px 14px', fontFamily: '"DM Sans", sans-serif', fontWeight: 300, fontSize: 12, color: 'var(--atelier-ink-soft)' }}>{v.city || '—'}</td>
                  <td style={{ padding: '11px 14px' }}>
                    <select value={v.tier} onChange={e => changeTier(v.id, e.target.value)} style={{ background: 'transparent', border: 'none', outline: 'none', fontFamily: '"Jost", sans-serif', fontWeight: 200, fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', color: v.tier === 'prestige' ? 'var(--atelier-ink)' : v.tier === 'signature' ? 'var(--role-metal)' : 'var(--atelier-ink-soft)', cursor: 'pointer' }}>
                  ...
                  <td style={{ padding: '11px 14px', fontFamily: '"DM Sans", sans-serif', fontWeight: 300, fontSize: 11, color: 'var(--atelier-ink-soft)', whiteSpace: 'nowrap' }}>{new Date(v.created_at).toLocaleDateString('en-IN')}</td>
                  <td style={{ padding: '11px 14px' }}>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                      <a href={`https://vendor.thedreamwedding.in/vendor/dashboard?id=${v.id}`} target="_blank" rel="noreferrer" style={{ fontFamily: '"Jost", sans-serif', fontWeight: 200, fontSize: 8, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--atelier-ink-soft)', textDecoration: 'underline' }}>View</a>
                      <button onClick={() => setConfirmRevoke(v.id)} style={{ background: 'none', border: 'none', fontFamily: '"Jost", sans-serif', fontWeight: 200, fontSize: 8, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--role-critical)', cursor: 'pointer' }}>Revoke</button>
                    </div>
                  </td>
                </tr>
              ))}
