"use client";
// app/w/leads/page.tsx — LEADS, INSIDE THE SHELL. R-38.1 + R-38.11.
//
// The room the enquiry desk fills. It is the first tile in the first band and the surface
// the founder walks first, so it is the one the crossing is judged on.
//
// ── WHAT CROSSED ────────────────────────────────────────────────────────────
// THE STRUCTURE. This route is a child of app/w/layout.tsx, so tapping the Leads tile
...
import LeadsSlice from '@/app/vendor/list/[slice]/leads';

export default function ShellLeadsPage() {
  ...
  return (
    <WorklistShell title={COPY.leadsTitle}>
      <RoomBody><LeadsSlice vendorId={session.id} /></RoomBody>
    </WorklistShell>
  );
}
