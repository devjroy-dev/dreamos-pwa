...
export default function DemoBusinessPage(){
  ...
  const{vendorName}=useDemoContext(handle);const base=`/demo/vendor/${handle}`;
  const LISTS:Item[]=[
  ...
  const STUDIO_ITEMS:Item[]=[
    {href:`${base}/studio/team`,label:'Team',desc:'roster and contact details',glyph:'T',locked:true},
    {href:`${base}/studio/tasks`,label:'Tasks',desc:'assignments and deadlines',glyph:'✓',locked:true},
    {href:`${base}/studio/team-payments`,label:'Team Payments',desc:'what you owe your crew',glyph:'◇',locked:true},
  ];
  function Row({item,_k}:{item:Item;_k?:React.Key}){
    const rowStyle:React.CSSProperties={display:'flex',alignItems:'center',padding:'16px 24px',gap:18,textDecoration:'none',borderBottom:'0.5px solid var(--atelier-card-border)',cursor:item.locked?'default':'pointer',opacity:item.locked?0.55:1};
    const inner=<><span style={{flexShrink:0,width:36,height:36,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:F.display,fontWeight:400,fontSize:26,color:A.brassWarm,lineHeight:1}}>{item.glyph}</span><div style={{flex:1,minWidth:0}}><div style={{fontFamily:F.script,fontWeight:500,fontSize:19,color:A.ink,letterSpacing:'0.005em',lineHeight:1.15}}>{item.label}</div><div style={{fontFamily:F.script,fontStyle:'italic',fontWeight:300,fontSize:12,color:A.inkMute,marginTop:2,letterSpacing:'0.01em'}}>{item.desc}</div></div>{item.locked?<span style={{fontFamily:F.label,fontWeight:400,fontSize:8,color:A.brass,letterSpacing:'0.32em',textTransform:'uppercase' as const,border:'0.5px solid rgba(201,168,76,0.5)',borderRadius:2,padding:'4px 9px',flexShrink:0}}>Prestige</span>:<Chevron/>}</>;
    if(item.locked)return<div style={rowStyle}>{inner}</div>;
  ...
  return(<div style={{flex:1,display:'flex',flexDirection:'column'}}>
    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',overflowY:'auto',overflowX:'hidden',paddingBottom:32}}>
      <SL label="Your Studio" first/>
      {LISTS.map(item=><div key={item.href}><Row item={item}/></div>)}
      <SL label="Team Hub"/>
      {STUDIO_ITEMS.map(item=><div key={item.href}><Row item={item}/></div>)}
      <div style={{padding:'24px 28px 8px'}}><div style={{fontFamily:F.script,fontStyle:'italic',fontWeight:300,fontSize:14,color:A.inkMute,lineHeight:1.55,textAlign:'center'}}>Team Hub is reserved for Prestige.<br/>Contact Swati to upgrade.</div></div>
    </div>
  </div>);
}
